...
def get_all_user_paths(root):
  ...
  try:
    xp = os.path.join(root, '{0}'.format(
        convert_paths('\\Documents and Settings')))
...
def SetupLogger(loggerName, logLevel, logFile):
  ...
  formatter = logging.Formatter(
      '%(asctime)s - %(levelname)-8s - %(name)s - %(module)s:%(funcName)s():%(lineno)d  | %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
  # Default TZ is local
...
if __name__ == '__main__':
  ...
  parser.add_argument('-i', '--input', action='store',
                      metavar='[Mount Point]', help='Path to mounted filesystem (e.g. - "E:" or "/mnt/point").', required=True)
  parser.add_argument('-o', '--output', action='store',
                      metavar='[Output Directory]', help='Output path to copy files to.', required=True)
  ...
  _logger = SetupLogger("AutoDFIR", logLevel, os.path.join(
      output_dir, 'sniper_process.log'))
