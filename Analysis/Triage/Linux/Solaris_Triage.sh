...
run_create_filelisting() {
  ...
  FILE_LISTING_CMD=(find / \( -type d -name export -print \) -o \( -type d -name oa -prune \) -o \( -type d -name od -prune \) -o \( -type d -name proc -prune \) -o -print)
...
run_copy_history_files() {
  ...
  for _history_file in "${HISTORY_FILES_TO_ACQUIRE[@]}"; do
    ...
    COPY_HIST_FILES_CMD=(find / \( -type d -name export -prune \) -o \( -type d -name proc -prune \) -o \( -type d -name oa -prune \) -o \( -type d -name od -prune \) -o -type f -name $_history_file -exec cp --parents {} . \;)
