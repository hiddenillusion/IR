...
# Pre-populated items
COMMANDS_TO_EXECUTE=("ps -PTcgjstUvZz" "ps -cvJnN" "lsof" "who -a" "df" "df -u" "df -h" "hostname" "vmware -v" "uname -a" "alias" "env" "esxcfg-advcfg -l" "esxcfg-fcoe -l" "esxcfg-fcoe -N" "esxcfg-ipsec -l" "esxcfg-ipsec -L" "esxcfg-module -l" "esxcfg-module -q" "esxcfg-mpath -l" "esxcfg-nas -l" "esxcfg-nics -l" "esxcfg-route -l" "esxcfg-route -n" "esxcfg-route -f V6 -l" "esxcfg-route -f V6 -n" "esxcfg-scsidevs -l" "esxcfg-vmknic -l" "esxcfg-volume -l" "esxcfg-vswitch -l" "esxcfg-info");
DIRECTORIES_TO_COPY=("/scratch/log");
FILES_TO_READ=("/etc/hosts" "/etc/os-release" "/proc/meminfo" "/proc/uptime" "/proc/version");
...
SPECIFIC_FILES_TO_ACQUIRE=("/etc/apache2.conf" "/etc/dnsmasq.conf" "/etc/fstab" "/etc/group" "/etc/hosts" "/etc/httpd.conf" "/etc/inetd.conf" "/etc/localtime" "/etc/mtab" "/etc/my.cnf" "/etc/passwd" "/etc/resolv.conf" "/etc/smb.cnf" "/etc/sshd_config" "/etc/sudoers" "/etc/syslog.conf" "/etc/timezone" "/var/log/access.log*" "/var/log/access_log*" "/var/log/audit.log*" "/var/log/btmp*" "/var/log/dmesg*" "/var/log/dpkg.log*" "/var/log/error.log*" "/var/log/error_log*" "/var/log/faillog*" "/var/log/apt/history.log*" "/var/log/httpd-error.log*" "/var/log/lastlog*" "/var/log/messages*" "/var/log/rpm.log*" "/var/log/secure*" "/var/log/syslog*" "/var/log/sudo*" "/var/log/utmp*" "/var/log/wtmp*" "/var/log/yum.log*");
